...
async function getRepos(): Promise<GithubRepo[]> {
  try {
    const res = await fetch(
      `https://api.github.com/users/${siteConfig.github.username}/repos?sort=updated&per_page=6&type=owner`,
      { next: { revalidate: 3600 } }
    );
    if (!res.ok) return [];
...
export default async function GithubPanel() {
  ...
  return (
    <section
      className="bg-bat-graphite py-24 px-8 md:px-16 lg:px-24 border-t border-bat-concrete"
      aria-label="GitHub"
    >
      <div className="max-w-6xl mx-auto">
            ...
            <div className="flex gap-10">
              {[
                ...
                <div key={label} className="flex flex-col gap-1">
                  <span className="font-display text-4xl text-bat-white">
                    {value}
                  </span>
        ...
        {/* Repos */}
        {repos.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border border-bat-concrete divide-y md:divide-y-0 md:divide-x divide-bat-concrete">
            {repos.map((repo) => (
              <a
                key={repo.id}
                href={repo.html_url}
                ...
                className="p-6 flex flex-col gap-3 hover:bg-bat-dark transition-colors duration-200 group"
              >
          ...
          <div className="border border-bat-concrete p-8">
            <p className="font-mono text-xs text-bat-ash tracking-wider">
              Update <code className="text-bat-red">src/config.ts</code> with
              your GitHub username to display repositories.
            </p>
          </div>
        )}
